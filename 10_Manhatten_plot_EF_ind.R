...
yearlist<-list(X11,X12,X12D,X13,X13D,X14,BLUP)
for (yi in 1:length(yearlist)){
  allmil<-as.data.frame(yearlist[[yi]])
...
EMp <- ggplot(EMallmil , aes(x=position, y=-log10(pval))) +
  #geom_point(pch=1) +
  geom_line()+
  ggtitle("Emily")+ 
...
FEp <- ggplot(FEallmil , aes(x=position, y=-log10(pval))) +
  #geom_point(pch=1) +
  geom_line()+
  ggtitle("Fenella")+ 
  #scale_color_manual(values=c('#7CAE00','#00BFC4',"#00BA38","#F8766D","#619CFF","#FF61CC","black","black"))+
...
Bp <- ggplot(Ballmil , aes(x=position, y=-log10(pval))) +
  #geom_point(pch=1) +
  geom_line()+
  ggtitle("Shared")+ 
  #scale_color_manual(values=c('#7CAE00','#00BFC4',"#00BA38","#F8766D","#619CFF","#FF61CC","black","black"))+
...
name<-paste0("MHP_ind_EF",yearlist[[yi]][1,18],".pdf")
pdf(name,width=12,height=9.6,paper='special') 
