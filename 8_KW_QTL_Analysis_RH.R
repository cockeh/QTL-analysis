...
pheno<-read.csv("rgxhlocM.csv")
basedir<-"./rxh_Hap/"
for (j in c(2:6)){
  score <- pheno[j]
  df = data.frame(lg=character(),Rname=character(),pval=double(),k=double(),sig=character(),mtype=character(),ll=double(),lm=double(),
  ...
  for (hg in 1:7)
  {
    ...
    for(sg in c("A","B","C","D"))
    {
      ...
      {
        i <- as.numeric(i + 1)
        mtype <- all_data[snp,1] #lmxll, nnxnp or hkxhk
        ...
        df[nrow(df) + 1, ] <- c(lg,Rname[[i]],p,k,sig,mtype,ll,lm,nn,np,hh,hk,kh,kk)
      }
    }
  }
  ...
  V1<-names(pheno[j])
  name<-paste("Sig_values",V1,".csv",sep="")
